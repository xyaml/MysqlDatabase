...
fclose(fd);
	...
	if (name[0] == '*')
	{
		sprintf(sql, "select stu.id, stu.name, stu.age, stu.ssex, stu.dept from stu, score where score.id=stu.id and stu.flag=1");
	}
	else
	{
		sprintf(sql, "select stu.id, stu.name, stu.age, stu.ssex, stu.dept from stu where name = '%s' and stu.flag=1", name);
	}
