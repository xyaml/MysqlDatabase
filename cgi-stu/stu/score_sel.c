...
fclose(fd);
	...
	if (name[0] == '*')
	{
		sprintf(sql, "select score.id, score.name, score.cno, score.score from score,stu where stu.id=score.id");
	}
	else
	{
		sprintf(sql, "select score.id, score.name, score.cno, score.score  from score,stu where name = '%s'", name);
	}
