...
fclose(fd);
	...
	if (name[0] == '*')
	{
		sprintf(sql, "select stu.id, stu.name, score.cno, class.cname from score,stu,class where stu.id=score.id and class.cno=score.cno  ");
	}
	else
	{
		sprintf(sql, "select stu.id, stu.name, score.cno, class.cname from score,stu,class where name = '%s'", name);
	}
